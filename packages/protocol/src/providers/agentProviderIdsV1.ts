...
//
// `kimi` and `fx` participate only through the generic `acpSessionList` direct-sessions source.
// Membership here is transport admission, not a capability claim: the authoritative declaration is
// `isBuiltInAcpSessionListingDeclared` in `@happier-dev/agents`, the daemon re-checks it in
// `validateDirectMachineSource`, and the live ACP handshake remains the runtime authority.
export const AGENT_PROVIDER_IDS_V1 = ['claude', 'codex', 'opencode', 'pi', 'kimi', 'fx'] as const;
