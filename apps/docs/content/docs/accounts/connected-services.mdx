...
**Usage limits** row can either track all of them (the default) or a selected
set. For example, a separate Spark or Fable allowance can have its own pool
without changing the model of any running session. Limits that an account is
not currently reporting stay visible in the selection and are treated as
