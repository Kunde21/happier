...
driving Happier**. If you are here to give your agent more tools, you want
[MCP servers](/extending/mcp-servers) instead.

Happier can run its own **external MCP server** so other MCP hosts — editors,
agents, automations — can control Happier:
...
This is separate from **MCP servers you manage inside Happier** (Playwright, GitHub, etc). Those are documented under [MCP servers](/extending/mcp-servers).
...
That host can execute enabled Happier actions, request approvals — and execute
...
Permission-intent inputs prefer `read_only`, `default`, `auto`, and `yolo`.
Older `read-only`, `safe-yolo`, `workspace_write`, `acceptEdits`, and
`bypassPermissions` aliases remain accepted for mixed-version compatibility.
...
`execution_run_start.permissionMode` accepts the same preferred permission
inputs. To preserve mixed-version execution-run contracts, Auto is returned and
stored using the established `workspace_write` wire value.
...
If you’re targeting a self-hosted server, ensure the MCP host launches Happier with the right server profile (or server URL flags). See [CLI → Server profiles](/apps/cli#server-profiles).
