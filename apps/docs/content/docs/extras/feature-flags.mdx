...
There are 76 features in the catalog. 22 of them have a switch you can
reach; the other 54 are decided before you get there.
...
| `connectedServices.autoDisablePlanInvalid` | Opt-in automatic pool-member disable after a model-entitlement failure. | server | `connectedServices.accountFallback` |
| `connectedServices.autoQuotaReset` | Opt-in banked quota reset spending after account pool exhaustion. | server | `connectedServices.accountFallback`, `connectedServices.quotas` |
| `connectedServices.poolQuotaLimitSelection` | Provider-limit-aware quota selection for connected service account pools. | server | `connectedServices.accountFallback`, `connectedServices.quotas` |
| `connectedServices.subscription` | Provider-optional subscription observations with independently fresh account-usage transport. | server | `connectedServices` |
