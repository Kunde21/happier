...
- **Read only** uses `ask`.
- **Safe YOLO** uses `smart`.
- **YOLO** uses `bypass`.
- **Plan** uses `plan`.

Happier can resume Devin sessions and switch among the live model choices reported by the CLI. Devin currently ignores MCP servers supplied in the standard ACP session request, so Happier delivers the built-in Happier tools and your selected MCP servers through a protected, process-scoped Devin MCP configuration. Your existing user-level Devin MCP servers are merged into that temporary configuration, and Happier removes it when the agent process ends. Happier does not modify the MCP configuration in your home directory or project.
