...
1. **Choose a Relay** (Happier Cloud is recommended to start; self-hosting is optional).
2. **Sign in / create an account** on that Relay.
3. **Use the desktop app or web client as your control panel.** Both can start
   and drive sessions; desktop can also install and manage the local background
   service.
...
1. Open the desktop app (recommended for local setup) or web app.
2. Choose **Happier Cloud** when the app asks you which Relay to use. It is a
   deliberate choice, not a migration of an account from another Relay.
3. Sign in.
...
2. Open the desktop app or web UI and choose that Relay.
   ...
   Use an address every device can actually reach. For Tailscale, each device
   must be on the same tailnet and the Relay must be published with Tailscale
   Serve or listen on a reachable Tailscale address; merely installing
   Tailscale is not enough. For LAN access, bind the Relay to a non-loopback
   address and allow its port through the host firewall. A Relay bound to
   `localhost` inside a VM is local to that VM, not to the Mac or PC hosting it.
...
2. Open the desktop app or web UI and choose that Relay (use the printed configure-server link/QR if available).
...
- Switching profiles selects another Relay and its account; it does not migrate an account, sessions, or settings between Relays.
