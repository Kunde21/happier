...
- Binary available on `PATH` as `opencode` or `opencode2` (or set `HAPPIER_OPENCODE_PATH`). Happier prefers the stable `opencode` name when both are installed; an explicit override is authoritative.

## Install OpenCode

If you use Happier's provider installer, you can install OpenCode from the CLI:
...
The server-native backend negotiates the installed server's V1 or V2 API over the same authenticated connection. Operations still supplied only by the legacy API—dynamic MCP registration, session diff, and session fork—remain available on dual-surface beta servers. A pure-V2 server fails those operations explicitly instead of sending unsupported requests.
...
1. Verify `opencode --version` or `opencode2 --version` works.
2. Restart Happier.
...
This usually means the session was detached back to remote mode from Happier. Reattach with `happier attach <session-id>` from a terminal on the same machine.
