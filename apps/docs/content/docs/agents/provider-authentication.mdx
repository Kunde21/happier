...
If the provider/backend supports Happier [Connected services](/accounts/connected-services), that is often the easiest way to make the target machine ready for handoff without depending only on a native CLI login on that machine.
