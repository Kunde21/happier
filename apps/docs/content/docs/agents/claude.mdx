...
| Runtime | How Happier drives Claude | Terminal to join? |
| --- | --- | --- |
| **Unified terminal** (opt-in) | Happier hosts Claude Code's own TUI in a terminal session and types prompts into its composer, so the app and the terminal share one Claude Code run. | Yes |
...
If you send a message from the app while Claude is locally controlled, Happier stores it in the pending queue first. When Claude is idle, Happier can switch back to remote control and process the queued message. If Claude is still working in the terminal, Happier waits until the local turn finishes or stops before switching, so the app message does not interrupt the current Claude turn.
...
- a **provider-agnostic permission intent** (`read_only`, `default`, `auto`, `yolo`)
...
Some permission behavior is implemented by Happier (tool approval gating) and can update immediately; other behavior depends on Claude’s own session flags and may only take effect when Claude is launched/resumed.

When local permission bridge is enabled, Claude permission prompts can still be forwarded into Happier while the terminal owns the session. Message sending is separate from permission approval: app messages sent during exclusive local control still go through the pending queue and are delivered only after Happier can safely resume remote processing.
...
### Let Happier install it

Happier can run the installer for you, using Anthropic's native installer rather than npm:
