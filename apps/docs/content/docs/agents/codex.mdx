---
title: Codex
description: Configure Codex in Happier, including the default App Server backend, installables, local control, and resume behavior.
---
...
Or install it yourself, then restart Happier:
...
Codex local control is exclusive. When the Codex TUI owns the session, Happier watches Codex turn boundaries and process state so it can tell when it is safe to resume remote processing. Completed turns, aborted turns, and provider process exit are treated as safe boundaries for handing control back to Happier.
...
If you send a message from the app while Codex is locally controlled, Happier stores it in the pending queue first. If Codex is idle, Happier can switch back to remote control and process it. If Codex is still running a local turn, Happier waits for that turn to finish, abort, or exit before switching, which avoids interrupting the current Codex TUI turn.
...
Happier uses a **provider-agnostic permission intent** (`read_only`, `default`, `auto`, `yolo`) and applies it to Codex in the best available way.
...
- Auto and YOLO control whether Happier prompts vs auto-approves actions it can approve.
...
Claude's plan mode is provider-native, so Happier gives it a dedicated control.
...
Because the list comes from the runtime, Happier shows the Mode control only when
...
Happier keeps the older spelling working: `--permission-mode plan` maps to
`--agent-mode plan`.
...
Happier's own review entry point uses the same path where the request can be
...
Where the running Codex build does not offer the review API, Happier reports the
