...
async function probeServerReadiness(params: Readonly<{ endpoint: string; token: string | null }>): Promise<ReadinessProbeResult> {
    ...
    // Running both sequentially cost every cold boot and every resume a second full round trip before the
    // socket transport was even constructed, without producing evidence the ping does not already carry.
    if (!params.token) {
        try {
