...
`docs/agent-craft.md` and `.agents/skills/handoff-report` own voice, ordering, epistemic clarity, and handoff style. This reference protects issue-specific content from omission; it is not an outline or field list.
...
Resolve the machine's ordinary authenticated `gh` login—not the transport identity exposed through `ghops`—and end every public issue comment with the standalone line `_Posted on behalf of @<resolved-login>._` This records the human authorization behind the transported comment without coupling the attribution to a particular bot or service account. Under exact authorization, treat the resolved handle as part of the payload shown for approval; under standing authorization, resolve it immediately before posting. Do not hardcode the handle or suppress it because the maintainer was mentioned earlier or may already be subscribed. If the local identity cannot be resolved, request authentication or an explicit attribution target before posting.
